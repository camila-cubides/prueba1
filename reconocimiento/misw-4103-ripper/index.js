...
(async () => {
  ...
  for (b of browsers) {
    ...
    await loginToGhost(page)

    for (const ghostPage of ghostPages) {
      try {
        console.log(`Navigating to ${ghostPage}`);
        const fullUrl = new URL(ghostPage, baseUrl + 'signin').toString();
        await recursiveExploration(page, fullUrl, 0, -1);
      } catch (error) {
        console.log(`Error exploring ${ghostPage}:`, error);
    ...
    try {
      ...
      // Optionally, suppress or log this error
...
async function recursiveExploration(page, link, depth, parentState) {
  ...
  if (!!body) {
    ...
    if (link.includes(baseUrl)) {
      ...
      // create method to get text areas
      ...
      for (const newLink of links) {
        if (!newLink.startsWith(baseUrl)) continue;
        if (visitedPages.has(newLink)) continue;
...
async function addListeners(page) {
  page.on("pageerror", async (err) => {
    const err_name = sanitizeFilename(err.toString());
    const capture_path = path.join(screenshots_directory, `error_${err_name}.png`);
    try {
...
async function interactWithObject(
  ...
  // add logic to interact with the object when is text area
...
async function elementScreenshot(location, currentState, page, moment) {
  ...
  const screenshotPath = path.join(
    screenshots_directory,
    `state_${currentState}_interaction_${statesDiscovered}${moment}.png`
  );
...
async function loginToGhost(page) {
  console.log("Logging ...");
